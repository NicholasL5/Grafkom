...
//            createBox();
    ...
    public void createEllipsoid2(){
        vertices.clear();
        ArrayList<Vector3f> temp = new ArrayList<>();

        for(double v = -Math.PI/2; v<= Math.PI/2; v+=Math.PI/60){
            for(double u = -Math.PI; u<= Math.PI; u+=Math.PI/60){
                float x = 1.0f * (float)(Math.cos(v) * Math.cos(u));
                float y = 0.5f * (float)(Math.cos(v) * Math.sin(u));
                float z = 0.5f * (float)(Math.sin(v));
                temp.add(new Vector3f(x,y,z));
            }
    ...
    public void createTriangularPrism(){
        vertices.clear();
        ArrayList<Vector3f> temp = new ArrayList<>();

        // Create the base isosceles triangle
        float baseSize = 0.5f;
        temp.add(new Vector3f(-baseSize/2, 0, 0));
        temp.add(new Vector3f(baseSize/2, 0, 0));
        temp.add(new Vector3f(0, baseSize*3, 0));

        // Extrude the triangle along the z-axis to create the prism
        float height = 1.0f;
        for (int i = 0; i < 3; i++) {
            Vector3f v1 = temp.get(i);
            Vector3f v2 = temp.get((i + 1) % 3);
            Vector3f v3 = new Vector3f(v1.x, v1.y, height);
            Vector3f v4 = new Vector3f(v2.x, v2.y, height);
