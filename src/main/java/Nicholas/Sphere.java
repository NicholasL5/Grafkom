...
//            createBox();
    ...
    public void createCone(){
        ...
        // Create top vertex
        temp.add(new Vector3f(0.0f, 0.0f, height));

        // Create triangles for sides
        for(int i = 0; i < temp.size() - 1; i++){
            Vector3f v1 = temp.get(i);
            Vector3f v2 = temp.get(i+1);
            Vector3f v3 = temp.get(temp.size()-1);
            vertices.add(v1);
            vertices.add(v2);
            vertices.add(v3);
        }
        ...
        for(int i = 0; i < temp.size() - 1; i++){
            Vector3f v1 = temp.get(i);
            Vector3f v2 = temp.get((i+1) % temp.size());
    ...
    public void Box(){
        ...
        for(double v = -Math.PI/2; v<= Math.PI/60; v+=Math.PI/60){
            for(double u = 45; u<= 360; u+=90f){
                float x = 1f * (float) Math.cos(Math.toRadians(u));
                float y = 0.1f * (float) Math.sin(Math.toRadians(u));
                float z = (float) v;
                temp.add(new Vector3f(x,y,z));
            }
        }
    ...
    public void elipticParaboloid(){
        ...
        for(double v = 1; v<= Math.PI/2.05; v+=Math.PI/720){
            for(double u = -Math.PI; u<= Math.PI; u+=Math.PI/720){
                float x = 1.5f* 0.05f * (float) v * (float) Math.cos(u);
                float y = 1.5f* 0.05f * (float) v * (float) Math.sin(u);
                float z = (float) v * (float) v ;
                temp.add(new Vector3f(x,y,z));
            }
        }
